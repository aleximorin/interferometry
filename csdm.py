...
if __name__ == '__main__':
    ...
    print(f'computing csdms from {t1} to {t2}')
    ...
    while ti < tend:
        ...
        try:
            toc = time()
            csd = interf.cross_spectral_density_matrix(seisclient, selected_channels, ti, 
                                  dt=86400, window_length_seconds=3600, 
                                  minf=MINF, maxf=MAXF, downsample=DOWNSAMPLE,
                                  overlap=0.5, nfbins=100, coordinates=coords, 
                                  onebit=True, whiten=True)
            if csd is not None:
                csd.to_netcdf(file, engine='h5netcdf')#, compute=False)
            tic = time()
